...
namespace	ST77xxConfig
{
	...
	constexpr	gpio_num_t
		PinSS		= GPIO_NUM_10,	//ST7735:CS, ST7789:CS
		PinDC		= GPIO_NUM_4,	//ST7735:A0, ST7789:DC
		PinRESET	= GPIO_NUM_NC;	//ST7735:RESET, ST7789:RST

	//コントローラー
	enum	class	EController	{ ST7735, ST7789 };	//←この行自体は編集不要。どれかを↓へ書く
	constexpr	auto	Controller = EController::ST7735;

	/*	ST7735-LCD ピン並び(top-view, pin-header top-side)
	[TOP-LEFT side] LED - SCK - SDA(MOSI) - A0(DC) - RESET - CS(SS) - GND - VCC(3V3) [TOP-RIGHT side]
	*/
}
